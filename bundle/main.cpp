...
void create_scene(std::shared_ptr<World> world) {
    ...
    occluder_ptr->set_sampler(world->sampler);
    ...
    world->sampler = std::make_shared<Jitter>(64, 83); //first number here is the number of samples
    world->max_depth = 2;
...
void render_fisheye() {
    ...
    fisheye_ptr->setEye({ 0, -200, 0 });
    fisheye_ptr->setLookAt({ 0,-100,-700 });
